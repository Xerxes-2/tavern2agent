...
> **本文档主要适用于轻量方案**。标准方案（CodeAct）下，常驻工具通常只有 `code_act` + `get_status` + `lookup` + `switch_toolset`，战斗 / 社交 / 制作 多步操作全部在沙箱里自由组合——不需要按场景动态切换工具组。下面「按场景分组」「专用工具优先于裸 patch」这些结论在**轻量方案**下仍适用；标准方案请跳到 `codeact.md` §三（三层 API）和 §六（protected paths）。

> **子代理工具集本节仍适用**——不论主 GM 是否走 CodeAct，subagent 都不拿 `code_act`（SKILL.md §三 multi-agent 决议），子代理工具集仍是本文档 §子代理工具集说的「轻量 / 只读」。
