...
GM 输出的 token 真人测试也得花（一样要让 GM 跑完），agent 玩家额外多花的只是自己回应那几百到几千 token，相对完整验证可以忽略。

### 7. 人类 spot check（推荐）

测试完后让用户读 `sessions/<id>.jsonl` 的关键回合即可——比让用户实时盯着整局玩省力得多。建议你（agent）在报告完成时附一段「**值得复核的 turn**」清单：
